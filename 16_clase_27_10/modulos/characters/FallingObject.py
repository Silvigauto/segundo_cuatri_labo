...
class FallingObject(Object):
    ...
    def move_down(self):
        if self.rect.y > self.screen[1]:
            self.set_random_position() #si llegaron al limite inferior de la pantalla se vuelven a posicionar
        else:
            super().move_down()
    ...
    def create_list(size, screen):
        ...
        for i in range(size):
            falling_object = FallingObject((50, 50), (0,0), screen) #el screen es para ubicarlo fuera de la pantalla
